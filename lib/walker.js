 ...
 */
Walker.prototype.go = async function(entry) {
  var that = this
  this._pending++

  try {
    let stat = await util.promisify(fs.lstat)(entry);
    if (stat.isDirectory()) {
      if (!that._filterDir(entry, stat)) {
        that.doneOne()
      } else {
        try {
          let files = await util.promisify(fs.readdir)(entry);

          that.emit('entry', entry, stat)
          that.emit('dir', entry, stat)
          for (const part of files){
            await that.go(path.join(entry, part))
          }
          that.doneOne()
        } catch (er) {
